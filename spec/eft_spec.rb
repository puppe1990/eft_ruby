...
describe EFT do
  ...
  describe '.tap_points' do
    it 'displays tapping points and phrases to the user' do
      ...
      expect do
        EFT.tap_points('anxiety',
                       EFT::Phrases::DEFAULT_PHRASES)
      end.to output(/Tap on the .* while focusing on the following issue: 'anxiety'/).and_output { |output|
               expect(output.strip).to match(/Tap on the .* while focusing on the following issue: 'anxiety'/)
             }
    end
  ...
  describe '.tap' do
    it 'starts a tapping session and prompts the user to enter their feelings' do
      ...
      expect do
        EFT.tap('anxiety',
                EFT::Phrases::DEFAULT_PHRASES)
      end.to output(/On a scale from 0 to 10, how do you feel now?/).to_stdout
    end
    ...
    it 'asks the user for a positive emotion or goal if the feeling is less than 10' do
      ...
      expect do
        EFT.tap('anxiety', EFT::Phrases::DEFAULT_PHRASES)
      end.to output(/Enter a positive emotion or goal: /).to_stdout
    end
    ...
    it 'taps on the points and displays the phrases to the user' do
      ...
      expect do
        EFT.tap('anxiety',
                EFT::Phrases::DEFAULT_PHRASES)
      end.to output(/Tap on the .* while focusing on the following issue: 'anxiety'/).to_stdout
    end
  end
end
